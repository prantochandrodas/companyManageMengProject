...
<head>
    ...
    <!-- Optional: DataTables Bootstrap 4 CSS -->
...
<body style="font-family: Arial, sans-serif;">
        ...
        <form style="background-color: #f0f0f0; padding:20px; margin:20px" action="{{ route('income.filter') }}">
            ...
            <div class="row">
                <div style="margin-bottom: 20px;" class="col-4">
                    ...
                    <select id="fund_category_id" name="fund_category_id" class="form-control" style="width: 100%; padding: 8px;">
                        <option value="">Select fund</option>
                        @foreach ($funds as $fund)
                        <option value="{{ $fund->id }}" {{request('fund_category_id') == $fund->id ? 'selected' :''}}>{{ $fund->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div style="margin-bottom: 20px;" class="col-4">
                    <label for="income_category_id" style="display: block; margin-bottom: 5px;">Income Category Name:</label>
                    <select id="income_category_id" name="income_category_id" class="form-control" style="width: 100%; padding: 8px;">
                ...
                <div style="margin-bottom: 20px;" class="col-4">
                    ...
                    <select id="income_head_id" name="income_head_id" class="form-control" style="width: 100%; padding: 8px;">
                        <option value="">Select Income head</option>
                    </select>
                </div>
                <div style="margin-bottom: 20px;" class="col-4">
                    <label for="formDate" style="display: block; margin-bottom: 5px;">Form date:</label>
                    <input type="date" id="formDate" name="formDate" value="{{ request('formDate') ?: now()->format('Y-m-d')}}" class="form-control" style="width: 100%; padding: 8px;">
                </div>
                <div style="margin-bottom: 20px;" class="col-4">
                    <label for="toDate" style="display: block; margin-bottom: 5px;">To date:</label>
                    <input type="date" id="toDate" name="toDate" class="form-control" style="width: 100%; padding: 8px;" value="{{request('toDate')}}">
                </div>
            </div>
            <button type="submit" style="background-color: #4CAF50; color: white; padding: 10px 20px; border: none; cursor: pointer; border-radius: 5px;">Filter</button>
        </form>
        ...
        <table id="expensesTable" class="table table-striped table-bordered" style="width:100%">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>@if($income->incomeHead){{ $income->incomeHead->name }} @else No income Head @endif</td>
    ...
    <!-- Optional: DataTables Bootstrap 4 JS -->
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            data.forEach(function(head) {
                                options += `<option value="${head.id}" ${head.id == '{{ request("income_head_id") }}' ? 'selected' : ''}>${head.name}</option>`;
                            });
